...
export async function createItem(
  ...
  try {
    ...
    // 5. Externalize base64 properties
...
export async function updateItem(
  ...
  const { actor, reason, expectedOv } = opts;
  const op = 'UPDATE';
...
export async function deleteItem(
  ...
  try {
    ...
    // 5. MongoDB transaction (no S3 writes for delete)
    let cv: number;
    let session: ClientSession | undefined;
